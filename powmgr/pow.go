...
func (q *Pow) Run(target uint64, obj []byte, donePowFunc func(obj []byte)) {
...
func (q *Pow) work() {
	for {
		...
		go func() {
			...
			obj := append(nonceBytes, order.object...)
			
			// Remove the last item from the queue.
			q.dequeue()
			
			// Do whatever we're supposed to do with this message.
			order.donePowFunc(obj)
		} ()
	}
}